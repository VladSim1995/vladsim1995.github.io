...
if (isset($_GET["logout"]))
  ...
  $_SESSION['urlpage'] = "<a href='index.php' >Главная</a> \ <a href='tovar.php' >Блюда</a>";
...
if (isset($type))
{
   switch ($type) {
        ...
        $url = "type=".clear_string($type)."&";
        $type = "WHERE type='".clear_string($type)."'"; 
  ...
  if (isset($action))
{
   ...
   switch ($action) {
    ...
    {
        $msgerror = 'У вас нет прав на удаление блюд!';
    }
	    break;
...
<ul id="options-list">
<li>Блюда</li>
<li><a id="select-links" href="#"><? echo $type_name; ?></a>
<div id="list-links" >
<ul>
<li><a href="tovar.php?type=all"><strong>Все блюда</strong></a></li>
<?php


 $result1 = mysql_query("SELECT * FROM category",$link);
...
do
{
 ...
 echo '<li><a href="tovar.php?type='.$row1["type"].'">'.$row1["type"].'</a></li>';   
...
<div id="block-info">
<p id="count-style">Всего блюд - <strong><?php echo $all_count_result ?></strong></p>
<p align="right" id="add-style"><a href="add_product.php">Добавить блюдо</a></p>
</div>
...
$count = mysql_query("SELECT COUNT(*) FROM table_products $type",$link);
...
if ($temp[0] > 0)   
{
$result = mysql_query("SELECT * FROM table_products $type ORDER BY products_id DESC LIMIT $start, $num",$link);
